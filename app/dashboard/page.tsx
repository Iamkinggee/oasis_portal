...
import { useTheme } from '@/app/components/ThemeProvider'; // ✅ FIXED: use existing theme hook
...
type Meeting = { id: number; title: string; date: string; saved: boolean };
type AttendanceRecord = { meeting_id: number; member_id: number; present: boolean };
type Celebration = { id: number; name: string; event: string; event_date: string }; // ✅ FIXED: matches actual DB schema
type Program = { id: number; title: string; date: string; venue: string };
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function DashboardPage() {
  // ✅ FIXED: use the shared ThemeProvider hook instead of polling localStorage
  const { isDark } = useTheme();

  // ✅ FIXED: get userId for scoped realtime
  ...
  const fetchAll = useCallback(async () => {
    const [
      ...
      supabase.from('meetings').select('id, title, date, saved').eq('saved', true).order('date'),
      supabase.from('attendance_records').select('meeting_id, member_id, present'),
      // ✅ FIXED: select event_date instead of day/month columns
      supabase.from('celebrations').select('id, name, event, event_date').order('event_date'),
      supabase.from('programs').select('id, title, date, venue').order('date'),
    ]);
  ...
  useEffect(() => {
    ...
    const tables = ['members', 'meetings', 'attendance_records', 'celebrations', 'programs'];
    const subs = tables.map(table =>
      supabase.channel(`dash-${table}:${userId}`)
        .on('postgres_changes', { event: '*', schema: 'public', table, filter: `user_id=eq.${userId}` }, fetchAll)
        .subscribe()
    );
    return () => { subs.forEach(s => supabase.removeChannel(s)); };
  }, [userId, fetchAll]);
  ...
  const monthlyData = (() => {
    ...
    return months.map(({ month, year, monthNum }) => {
      ...
      return { month, pct: Math.round((totalPresent / (mtgs.length * totalMembers)) * 100) };
    });
  })();
  ...
  // ✅ FIXED: filter celebrations by parsing event_date instead of .month field
  const currentMonth = new Date().getMonth() + 1;
  const currentCelebrations = celebrations
    .filter(c => {
      if (!c.event_date) return false;
      return new Date(c.event_date + 'T00:00:00').getMonth() + 1 === currentMonth;
    })
    .slice(0, 5);
          ...
          {upcomingPrograms.length === 0
            ...
            : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {upcomingPrograms.map((ev, i) => (
                  <div key={ev.id}
                    style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px', borderRadius: 10, transition: 'background 0.15s' }}
                    onMouseEnter={e => (e.currentTarget.style.background = rowHov)}
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                  >
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', minWidth: 0 }}>
        ...
        <DashCard isDark={isDark}>
          ...
          {currentCelebrations.length === 0
            ...
            : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginTop: 6 }}>
                {currentCelebrations.map((c, i) => {
                  // ✅ FIXED: parse day from event_date instead of c.day
                  const day = new Date(c.event_date + 'T00:00:00').getDate();
                  return (
                    <div key={c.id}
                      style={{ display: 'grid', gridTemplateColumns: '1fr 50px', padding: '10px 10px', borderRadius: 10, transition: 'background 0.15s' }}
                      onMouseEnter={e => (e.currentTarget.style.background = rowHov)}
                      onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                    >
                      <div>
