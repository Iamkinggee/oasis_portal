...
function formatDate(iso: string) {
  if (!iso) return '';
  return new Date(iso + 'T00:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'long', day: 'numeric', year: 'numeric' }).toUpperCase();
}
function formatDateShort(iso: string) {
  if (!iso) return '';
  const d = new Date(iso + 'T00:00:00');
...
export default function AttendancePage() {
  ...
  const fetchMeetings = useCallback(async () => {
    ...
    if (error || !md) return;
    const { data: ad } = await supabase.from('attendance_records').select('*');
    const enriched: Meeting[] = md.map(m => ({ ...m, attendance: (ad || []).filter((a: any) => a.meeting_id === m.id) }));
    setMeetings(enriched);
    setActiveMeetingId(prev => {
      if (prev !== null) return prev;
      const last = enriched[enriched.length - 1];
  ...
  useEffect(() => {
    if (!userId) return;
    const s1 = supabase.channel(`meetings-rt:${userId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'meetings', filter: `user_id=eq.${userId}` }, fetchMeetings)
      .subscribe();
    const s2 = supabase.channel(`attendance-rt:${userId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'attendance_records', filter: `user_id=eq.${userId}` }, fetchMeetings)
      .subscribe();
    const s3 = supabase.channel(`members-rt-att:${userId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'members', filter: `user_id=eq.${userId}` }, fetchMembers)
      .subscribe();
    return () => { supabase.removeChannel(s1); supabase.removeChannel(s2); supabase.removeChannel(s3); };
  }, [userId, fetchMeetings, fetchMembers]);
  ...
  const handleSave = async () => {
    if (!activeMeeting || activeMeeting.saved || !userId) return;
    setSaving(true);
  ...
  const card: React.CSSProperties = { background: cardBg, border: `1px solid ${cardBd}`, borderRadius: 16, overflow: 'hidden', transition: 'background 0.3s, border-color 0.3s' };
  const cardHead: React.CSSProperties = { background: headBg, padding: isMobile ? '12px 14px' : '14px 20px', borderBottom: `1px solid ${dividerBd}` };
  const cardBody: React.CSSProperties = { padding: isMobile ? '10px 10px 14px' : '12px 16px 16px' };
  const sectionLabel: React.CSSProperties = { fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: textMuted };
  const colW = isMobile ? '46px' : '72px';
  const btnSize = isMobile ? 30 : 26;
  const gridCols = isDesktop ? 'minmax(0,1fr) 300px' : '1fr';
  const pagePad  = isMobile ? '14px 12px 150px' : isTablet ? '20px 20px 120px' : '24px 24px 100px';

  const CollapsibleHead = ({ label, collapsed, onToggle }: { label: React.ReactNode; collapsed: boolean; onToggle: () => void }) => (
    <div style={{ ...cardHead, cursor: 'pointer', userSelect: 'none' as const }} onClick={onToggle}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>{label}</div>
        {collapsed ? <ChevronRight size={14} color={textSub} /> : <ChevronDown size={14} color={textSub} />}
  ...
  return (
    <div style={{ background: pageBg, minHeight: '100vh', padding: pagePad, transition: 'background 0.3s', boxSizing: 'border-box' }}>
      <style>{`@keyframes spin { to { transform: rotate(360deg) } } .no-scroll::-webkit-scrollbar { display: none; } .no-scroll { -ms-overflow-style: none; scrollbar-width: none; }`}</style>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: gridCols, gap: 14, alignItems: 'start' }}>
          ...
          <div style={card}>
            <div style={cardHead}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>
                ...
                {activeMeeting?.saved && (
                  <span style={{ flexShrink: 0, padding: '2px 9px', borderRadius: 20, background: '#dcfce7', color: '#15803d', border: '1px solid #86efac', fontSize: 11, fontWeight: 600 }}>Saved ✓</span>
                )}
              </div>
              {meetings.length > 1 && (
                <div className="no-scroll" style={{ marginTop: 10, display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 2 }}>
                  {meetings.map(m => (
                    <button key={m.id} onClick={() => setActiveMeetingId(m.id)}
                      style={{ flexShrink: 0, fontSize: 10, fontWeight: 600, padding: isMobile ? '5px 9px' : '5px 10px', borderRadius: 8, border: `1px solid ${m.id === activeMeetingId ? (isDark ? '#f3f4f6' : '#111827') : cardBd}`, background: m.id === activeMeetingId ? (isDark ? '#f3f4f6' : '#111827') : cardBg, color: m.id === activeMeetingId ? (isDark ? '#111827' : '#ffffff') : textMuted, cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', touchAction: 'manipulation' }}>
                      {m.title} · {formatDateShort(m.date)}
                    </button>
                  ))}
                </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {/* History */}
            <div style={card}>
              <CollapsibleHead collapsed={collapsedHistory} onToggle={() => setCollapsedHistory(v => !v)} label={<span style={sectionLabel}>History</span>} />
              {!collapsedHistory && (
                <div style={{ ...cardBody, display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {savedMeetings.length === 0 ? (
                    <p style={{ fontSize: 12, color: textSub, margin: 0 }}>No saved meetings yet.</p>
                    ...
                    return (
                      <div key={m.id} style={{ position: 'relative' }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, alignItems: 'stretch' }}>
                          <button onClick={() => setHistoryView(m)}
                            style={{ flex: 1, textAlign: 'left', borderRadius: 12, border: `1px solid ${isActive ? (isDark ? '#f3f4f6' : '#111827') : cardBd}`, background: isActive ? (isDark ? '#f3f4f6' : '#111827') : cardBg, padding: '10px 12px', cursor: 'pointer', fontFamily: 'inherit', boxSizing: 'border-box', minWidth: 0, touchAction: 'manipulation' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                              <span style={{ fontSize: 11, fontWeight: 700, color: isActive ? (isDark ? '#111827' : '#ffffff') : textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                {m.title.toUpperCase()} – {formatDateShort(m.date)}
                              </span>
                          ...
                          <button onClick={e => { e.stopPropagation(); setConfirmDeleteId(m.id); }} title="Delete meeting"
                            style={{ flexShrink: 0, width: isMobile ? 40 : 36, borderRadius: 12, border: `1px solid ${cardBd}`, background: cardBg, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: textSub, touchAction: 'manipulation' }}
                            onMouseEnter={e => { e.currentTarget.style.borderColor = '#ef4444'; (e.currentTarget as HTMLElement).style.color = '#ef4444'; }}
                            onMouseLeave={e => { e.currentTarget.style.borderColor = cardBd; (e.currentTarget as HTMLElement).style.color = textSub; }}>
                            <Trash2 size={13} />
                          </button>
            ...
            {/* Most Consistent */}
            <div style={card}>
              <CollapsibleHead collapsed={collapsedConsistent} onToggle={() => setCollapsedConsistent(v => !v)}
                label={<><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e', display: 'inline-block', flexShrink: 0 }} /><span style={sectionLabel}>Most Consistent</span></>} />
              {!collapsedConsistent && (
                <div style={cardBody}>
                  <p style={{ fontSize: 11, color: textSub, margin: '0 0 8px' }}>
                    Attended all of last {consistencyStats.meetingCount} saved meeting{consistencyStats.meetingCount !== 1 ? 's' : ''}
            ...
            {/* Most Inconsistent */}
            <div style={card}>
              <CollapsibleHead collapsed={collapsedInconsistent} onToggle={() => setCollapsedInconsistent(v => !v)}
                label={<><span style={{ width: 8, height: 8, borderRadius: '50%', background: '#ef4444', display: 'inline-block', flexShrink: 0 }} /><span style={sectionLabel}>Most Inconsistent</span></>} />
              {!collapsedInconsistent && (
                <div style={cardBody}>
                  <p style={{ fontSize: 11, color: textSub, margin: '0 0 8px' }}>
                    Missed all of last {consistencyStats.meetingCount} saved meeting{consistencyStats.meetingCount !== 1 ? 's' : ''}
      ...
      <Dialog open={newMeetingOpen} onOpenChange={setNewMeetingOpen}>
        <DialogContent style={{ background: cardBg, border: `1px solid ${cardBd}`, borderRadius: 18, padding: 0, width: isMobile ? 'calc(100vw - 24px)' : '420px', maxWidth: '96vw', display: 'flex', flexDirection: 'column', color: textPrimary }}>
          ...
          <div style={{ padding: '12px 18px 20px', borderTop: `1px solid ${dividerBd}`, display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <button disabled={!newTitle.trim() || !newDate || creating || !userId} onClick={handleCreateMeeting}
              style={{ height: 44, padding: '0 20px', borderRadius: 8, border: 'none', background: (!newTitle.trim() || !newDate || creating || !userId) ? (isDark ? '#374151' : '#e5e7eb') : '#059669', color: (!newTitle.trim() || !newDate || creating || !userId) ? textSub : '#fff', fontSize: 13, fontWeight: 600, cursor: (!newTitle.trim() || !newDate || creating || !userId) ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6, touchAction: 'manipulation' }}>
              {creating ? <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Creating…</> : 'Create Meeting'}
            </button>
          </div>
        </DialogContent>
      ...
      <Dialog open={!!historyView} onOpenChange={() => setHistoryView(null)}>
        <DialogContent style={{ background: cardBg, border: `1px solid ${cardBd}`, borderRadius: 18, padding: 0, width: isMobile ? 'calc(100vw - 24px)' : '460px', maxWidth: '96vw', maxHeight: isMobile ? '90vh' : '85vh', display: 'flex', flexDirection: 'column', color: textPrimary }}>
          ...
          <div style={{ overflowY: 'auto', flex: 1, padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 5 }}>
            {members.map((member, idx) => {
              const present = historyView?.attendance.find(a => a.member_id === member.id)?.present;
              return (
                <div key={member.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '9px 10px' : '8px 12px', borderRadius: 10, background: present ? rowPresent : (isDark ? 'rgba(239,68,68,0.08)' : '#fef2f2'), gap: 8 }}>
