...
//     const { data: members } = await supabase.from("members").select("id, name, phone, gender").ilike("name", `%${q}%`).limit(5);
//     const memberResults: SearchResult[] = (members ?? []).map(m => ({
//       id: `m-${m.id}`, label: m.name, sub: `${m.gender ?? "Member"} · ${m.phone ?? ""}`,
//       href: "/dashboard/members", category: "Members", icon: "👤",
//     }));

//     const { data: programs } = await supabase.from("programs").select("id, title, date, venue").ilike("title", `%${q}%`).limit(4);
//     const programResults: SearchResult[] = (programs ?? []).map(p => ({
//       id: `pr-${p.id}`, label: p.title, sub: `${p.date ?? ""} · ${p.venue ?? ""}`,
//       href: "/dashboard/programs", category: "Programs", icon: "⭐",
//     }));

//     const { data: meetings } = await supabase.from("meetings").select("id, title, date").ilike("title", `%${q}%`).limit(3);
//     const meetingResults: SearchResult[] = (meetings ?? []).map(m => ({
//       id: `mt-${m.id}`, label: m.title, sub: `Meeting · ${m.date ?? ""}`,
//       href: "/dashboard/attendance", category: "Attendance", icon: "📋",
//     }));

//     const { data: celebrations } = await supabase.from("celebrations").select("id, name, event").ilike("name", `%${q}%`).limit(3);
//     const celebResults: SearchResult[] = (celebrations ?? []).map(c => ({
//       id: `c-${c.id}`, label: c.name, sub: c.event ?? "Celebration",
...
//         zIndex: 500, fontFamily: "'DM Sans', system-ui, sans-serif",
//       }}>
//         {searching ? (
...
//                   onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
//                 >
//                   <div style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0, background: sbg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>
//                     {r.icon}
//                   </div>
//                   <div style={{ flex: 1, minWidth: 0 }}>
...
//           /* On desktop the sidebar shows the logo — hide it from topbar */
...
//             position: "absolute", top: 0, left: 0, right: 0,
//             background: bg, borderBottom: `1px solid ${bd}`,
//             padding: "10px 14px", display: "flex", gap: 10, alignItems: "center", zIndex: 491,
//           }} onClick={e => e.stopPropagation()}>
//             <svg viewBox="0 0 24 24" fill="none" strokeWidth={2} stroke={ic} style={{ width: 16, height: 16, flexShrink: 0 }}>
//               <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
//             </svg>
...
//         {/* ── Mobile-only logo (left side, hidden on desktop where sidebar shows it) ── */}
...
//             <span style={{
//               width: 24, height: 24, background: "#111", borderRadius: 5,
//               display: "grid", gridTemplateColumns: "1fr 1fr", gridTemplateRows: "1fr 1fr",
//               gap: 3, padding: 4, flexShrink: 0,
//             }}>
//               {[0,1,2,3].map(i => (
...
//         <button id="topbar-search-mobile-btn"
//           onClick={() => setSearchOpen(true)}
//           style={{ width: 36, height: 36, borderRadius: 9, border: "none", background: "transparent", cursor: "pointer", alignItems: "center", justifyContent: "center", flexShrink: 0, marginLeft: "auto" }}
//           onMouseEnter={e => (e.currentTarget.style.background = sbg)}
//           onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
//         >
//           <svg viewBox="0 0 24 24" fill="none" strokeWidth={2} stroke={ic} style={{ width: 18, height: 18 }}>
...
//             {notifOpen && (
//               <div style={{
//                 position: "fixed", right: 14, top: 68,
//                 width: "min(340px, calc(100vw - 28px))",
//                 borderRadius: 16, background: panelBg, border: `1px solid ${panelBd}`,
//                 boxShadow: isDark ? "0 24px 60px rgba(0,0,0,0.65)" : "0 24px 60px rgba(0,0,0,0.14)",
//                 overflow: "hidden", zIndex: 200, animation: "slideUp 0.18s ease",
//               }}>
...
//                 padding: "5px 10px 5px 5px",
//                 background: userBg, borderRadius: 11, flexShrink: 0,
//                 transition: "background 0.15s, box-shadow 0.15s", cursor: "pointer",
//               }}
//               onMouseEnter={e => {
          ...
          Object.entries(grouped).map(([cat, items]) => (
            <div key={cat}>
              ...
              {items.map(r => (
                <div key={r.id}
                  onClick={() => {
                    // Navigate to the page and pass the search query + record id as params
                    // so the destination page can highlight/filter to the matching record
                    const params = new URLSearchParams();
                    params.set("q", search.trim());
                    // Pass the raw record id (strip category prefix) for direct highlighting
                    const rawId = r.id.replace(/^(m-|pr-|mt-|c-)/, "");
                    if (!r.id.startsWith("p-")) params.set("highlight", rawId);
                    router.push(`${r.href}?${params.toString()}`);
                    setSearch(""); setShowResults(false); setSearchOpen(false);
                  }}
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", cursor: "pointer", transition: "background 0.12s" }}
                  onMouseEnter={e => (e.currentTarget.style.background = hoverBg)}
                  onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
    ...
    <>
            ...
            <input
              ...
              onKeyDown={e => {
                if (e.key === "Enter" && results.length > 0) {
                  ...
                  const params = new URLSearchParams();
                  params.set("q", search.trim());
                  const rawId = first.id.replace(/^(m-|pr-|mt-|c-)/, "");
                  if (!first.id.startsWith("p-")) params.set("highlight", rawId);
                  router.push(`${first.href}?${params.toString()}`);
                  setSearch(""); setShowResults(false); setSearchOpen(false);
                }
              }}
              placeholder="Search anything — members, programs, pages…"
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, marginLeft: "auto" }}>
          ...
          {/* Mobile search icon — lives here with other icons, hidden on desktop */}
          <button id="topbar-search-mobile-btn"
            onClick={() => setSearchOpen(true)}
            style={{ width: 36, height: 36, borderRadius: 9, border: "none", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
            onMouseEnter={e => (e.currentTarget.style.background = sbg)}
